...
export function generateFinancialPDFReport(financialSummary, transactions = [], filters = {}) {
  const printWindow = window.open('', '_blank', 'width=950,height=1000')
  if (!printWindow) {
    alert('Please allow popups for this site to generate the PDF report.')
    return
  ...
  const reportTitle = filters.projectFilter
    ? `Financial Report - ${filters.projectFilter}`
    : 'Comprehensive Financial & Payment Report'
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Inter', sans-serif; color: #1e293b; background: #fff; padding: 28px; font-size: 12px; line-height: 1.5; }
    .header { display: flex; justify-content: space-between; border-bottom: 3px solid #2f5d50; padding-bottom: 12px; margin-bottom: 20px; }
    .brand { font-size: 20px; font-weight: 800; color: #2f5d50; }
    .title-badge { font-size: 14px; font-weight: 700; color: #fff; background: #2f5d50; padding: 6px 14px; border-radius: 6px; }
    ...
    th { background: #2f5d50; color: #fff; text-align: left; padding: 8px 10px; font-weight: 700; }
    td { padding: 8px 10px; border-bottom: 1px solid #e2e8f0; }
    tr:nth-child(even) { background: #f8fafc; }
    .text-right { text-align: right; }
    .footer { margin-top: 30px; display: flex; justify-content: space-between; font-size: 11px; color: #64748b; border-top: 1px solid #e2e8f0; padding-top: 14px; }
  </style>
</head>
<body>
  <div class="header">
    <div>
      <div class="brand">CONSTRUCTION PMS PRO</div>
      <div style="font-size: 10px; color: #64748b; font-weight: 600;">PROJECT FINANCIAL & PAYMENTS STATEMENT</div>
    </div>
    <div style="text-align: right;">
      <div class="title-badge">${reportTitle}</div>
  ...
  <div class="stat-cards">
    <div class="card">
      ...
      <div class="card-val green">${formatCurrencyINR(financialSummary.totalIncome || 0)}</div>
    </div>
    <div class="card">
      <div class="card-label">Total Expense</div>
      <div class="card-val red">${formatCurrencyINR(financialSummary.totalExpense || 0)}</div>
    </div>
    <div class="card">
      <div class="card-label">Net Profit</div>
      <div class="card-val gold">${formatCurrencyINR(financialSummary.netProfit || 0)}</div>
    </div>
    <div class="card">
      <div class="card-label">Profit Margin</div>
      <div class="card-val gold">${(financialSummary.profitMargin || 0).toFixed(2)}%</div>
    </div>
  </div>
  ...
  <table>
    ...
    <tbody>
      ${transactions.map((t) => `
        <tr>
          <td><strong>${t.payment_number || '-'}</strong></td>
          <td style="color: ${t.payment_type === 'Income' ? '#15803d' : '#b91c1c'}; font-weight: 700;">${t.payment_type}</td>
          <td>${t.project_name || '-'}</td>
          <td>${t.party_name || '-'}</td>
          <td>${formatDate(t.payment_date)}</td>
          <td>${t.payment_method || '-'}</td>
          <td>${t.status || '-'}</td>
          <td class="text-right" style="font-weight: 700;">${formatCurrencyINR(t.amount || 0)}</td>
        </tr>
      `).join('')}
    </tbody>
  </table>
  ...
  <div class="footer">
    <div>Generated by Construction PMS System</div>
    <div>Page 1 of 1</div>
  </div>
